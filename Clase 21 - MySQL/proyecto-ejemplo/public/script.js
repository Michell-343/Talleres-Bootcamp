...
userForm.addEventListener('submit', async (e) => {
  ...
  const response = await fetch('/api/users/create', {
    ...
    body: JSON.stringify({ nombre, email }), // Convierte los datos del formulario en un objeto JSON y los envía en el cuerpo de la solicitud.
  });
...
window.editUser = async (id) => {
  ...
  const response = await fetch(`/api/users/edit/${id}`, {
    ...
    body: JSON.stringify({ nombre, email }), // Envía los datos actualizados en formato JSON.
  });
